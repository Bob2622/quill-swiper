...
class SwpierBlot extends BlockEmbed {
  ...
  static renderSwiper(node, options) {
    // empty content
    node.innerHTML = ''; // set swiper style
  ...
  static showEditDialog(node, options) {
    ...
    if (dialogDom === null) {
      ...
      dialogDom = node.querySelector('.swiper-edit-dialog'); // close dialog
  ...
  optimize(context) {// console.log('this is optimize')
...
SwpierBlot.blotName = 'swiper';
SwpierBlot.tagName = 'div';
SwpierBlot.options = {
  width: '100%',
  height: 'auto',
  imgs: []
...
const parseDataUrl = function (fileInput, options) {
  ...
  Promise.all(fileReadpromises).then(res => {
    trigger.call(this, res.map(file => ({
      url: file
    })), options.uploadResFormat, fileInput);
  });
}; // upload img
